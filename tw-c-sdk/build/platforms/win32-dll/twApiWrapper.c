...
#else
	#pragma comment (lib, "axtls_lib.lib")
	#pragma comment (lib, "twSDK.lib")
#endif
...
#define NOTIFYPROPERTYUPDATE_SERVICE "NotifyPropertyUpdate"
								...
								uint32_t msgHandlerThreadCount, uint32_t apiTaskerThreadCount)
	{
		...
		code = twApi_SetOfflineMsgStoreDir(offlineMsgStoreDir);
		if (code) { return code; }
	...
	int twWApi_RegisterServices(enum entityTypeEnum entityType, char *entityName, twWRegInfoList *regList, void *userdata, char autoRegisterStandardServices)
	{
		...
		if (autoRegisterStandardServices == 1) {
			...
			// unregister the builtin GetMetadata service from twSDK, and register our own
			twApi_UnregisterServiceCallback(entityName, GETMETADATA_SERVICE, NULL);
...
#if defined(ENABLE_FIPS_MODE)
		twTunnelManager_EnableFipsMode(1);
#endif
								...
								char *stagingDir, fileTransfer_cb cb)
	{
		...
		twcfg.file_xfer_staging_dir = stagingDir;
	...
	int twWDataBlock_FromStruct(void *structPtr, char isPrimitive, twWDataBlock **outBlock)
	{
		...
		out = CoTaskMemAlloc(sizeof(twWDataBlock));
		if (!out) {
	...
	{
		if (block) {
			if (block->bytesPtr) { CoTaskMemFree(block->bytesPtr); }
			CoTaskMemFree(block);
			block = 0;
		}
	}
