...
twThread * tunnelManagerThread = NULL;
...
int main( int argc, char** argv ) {
	...
	*/
	...
	twTunnelManager_Create(TW_HOST,port,TW_APP_KEY, TRUE);
	...
	fileWatcherThread = twThread_Create(fileWatcherTask, FILE_WATCHER_RATE_MSEC, NULL, TRUE); 
	/* Create and start a thread for the tunnel manager function */
	tunnelManagerThread = twThread_Create(twTunnelManager_TaskerFunction, 5, NULL, TRUE);

	/* Check for any errors */
	if (!apiThread || !dataCollectionThread || !fileWatcherThread || !tunnelManagerThread) {
	...
	TW_LOG(TW_DEBUG,"main: Data Collection Thread isRunning: %s", twThread_IsRunning(dataCollectionThread) ? "TRUE" : "FALSE");
	TW_LOG(TW_DEBUG,"main: File Watcher Thread isRunning: %s", twThread_IsRunning(fileWatcherThread) ? "TRUE" : "FALSE");
	TW_LOG(TW_DEBUG,"main: Tunnel Manager Thread isRunning: %s", twThread_IsRunning(tunnelManagerThread) ? "TRUE" : "FALSE");
	{
	...
	twThread_Delete(tunnelManagerThread);
