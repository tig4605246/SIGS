...
char * createDigestCredentials(twSocket * s, char * hdr) {
	...
	*/
	...
	//Create HA1  
	twMD5_Init(&md5);
	twMD5_Update(&md5, (unsigned char *)s->proxyUser, strlen(s->proxyUser));
	twMD5_Update(&md5, ":", strlen(":"));
	twMD5_Update(&md5, (unsigned char *)realm, strlen(realm));
	twMD5_Update(&md5, ":", strlen(":"));
	twMD5_Update(&md5, (unsigned char *)s->proxyPass, strlen(s->proxyPass));
	...
	//Create HA2
	twMD5_Init(&md5);
	twMD5_Update(&md5, (unsigned char *)"CONNECT:", strlen("CONNECT:"));
	twMD5_Update(&md5, (unsigned char *)s->host, strlen(s->host));
	twMD5_Update(&md5, ":", strlen(":"));
	snprintf(mdHA2String, 8, "%d", s->port);
	twMD5_Update(&md5, (unsigned char *)mdHA2String, strlen(mdHA2String));
	...
	twMD5_Init(&md5);
	twMD5_Update(&md5, (unsigned char *)mdHA1String, strlen(mdHA1String));
	twMD5_Update(&md5, ":", strlen(":"));
	twMD5_Update(&md5, (unsigned char *)nonce, strlen(nonce));
	twMD5_Update(&md5, ":", strlen(":"));
	twMD5_Update(&md5, "00000001", strlen("00000001"));
	twMD5_Update(&md5, ":", strlen(":"));
	twMD5_Update(&md5, (unsigned char *)cnonce, 8);
	twMD5_Update(&md5, (unsigned char *)":auth:", strlen(":auth:"));
	twMD5_Update(&md5, (unsigned char *)mdHA2String, strlen(mdHA2String));
...
int connectToProxy(twSocket * s, char * authCredentials) {
	...
	resp = (char *)TW_CALLOC(REQ_SIZE, 1);
	if (!resp) {
		TW_FREE(req);
		return TW_ERROR_ALLOCATING_MEMORY;
	} 
	...
	res = twSocket_Write(s, req, strlen(req), twcfg.connect_timeout);
	if (res != strlen(req)) {
		TW_FREE(req);
		TW_FREE(resp);
	...
	if (strcmp(respCode, "407") == 0 && !authCredentials) {
		...
		if (strcmp(supportAuthTypes[i], "ntlm") == 0) {
			...
			s->proxyHost = tmpHost;
			/* Ntlm Auth */
			res = NTLM_connectToProxy(s, req, resp, s->proxyUser, s->proxyPass);
		} else 
#endif
		if (strcmp(supportAuthTypes[i], "digest") == 0) {
			...
			/* Basic Auth */
			unsigned long len = (strlen(s->proxyUser) + 1 + strlen(s->proxyPass)) * 2 + strlen("Proxy-Authorization: Basic ");
			char * encodedAuth = 0;
			...
			if (base64_encode((unsigned char *)auth, strlen(auth), (unsigned char *)encodedAuth, &len)) {
				if (auth) TW_FREE(auth);
				if (encodedAuth) TW_FREE(encodedAuth);
				TW_FREE(resp);
				TW_FREE(req);
