...
/**
 * \brief Shuts down a tunnel.
 *
 * \param[in]     id        The ID of the tunnel to shutdown.
 ...
 * \param[in]     proxyPass     The password to supply to the proxy (NULL if
 *                              authentication is disabled).
 *
 * \return #TW_OK if successful, positive integral on error code (see
 * twErrors.h) if an error was encountered.
...
/**
 * \brief Passthru to enable FIPS mode for TLS providers that support to.
 *
 * \return #TW_OK if successful, positive integral on error code (see
 * twErrors.h) if an error was encountered.
 *
