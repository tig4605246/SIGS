...
char * adjustPath(const char * entityName, char * path, char fromStartTransfer) {
	...
	*/
	...
	size_t stageDir = fromStartTransfer ? 0 : strlen(TW_VIRTUAL_STAGING_DIR) + 1;
	...
	adjustedPath = (char *)TW_CALLOC(strlen(newPath) + 1 + strlen(entityName) + strlen(TW_VIRTUAL_STAGING_DIR) + 6, 1);
	if (!adjustedPath) {
		TW_LOG(TW_ERROR, "adjustPath: Unable to allocate memory for adjustedPath.");
		return NULL;
	}
	/* Start with the staging dir */
	strcat(adjustedPath,TW_VIRTUAL_STAGING_DIR);
	strcat(adjustedPath, TW_FILE_DELIM_STR);
	if (fromStartTransfer) {
		/* Check to see if this is an old or new server by looking for '.part' */
		if(entityName[0] == '*') {
			strcat(adjustedPath, &entityName[1]);
		} else {
			strcat(adjustedPath, entityName);
		}
		strcat(adjustedPath, newPath);
		...
		if (dotPart && !strcmp(dotPart,".part")) {
			...
			strcat(adjustedPath, ".part");
		}
	} else {
		strcat(adjustedPath, entityName);
		strcat(adjustedPath, ".");
		strcat(adjustedPath, newPath + strlen(TW_VIRTUAL_STAGING_DIR) + 1);
	}
...
int getPaths(twInfoTable * params, const char * entityName, char ** path, char ** realPath,
			 ...
			 char ** adjustedPath, char ** realAdjustedPath, char fromStartTransfer) {
...
enum msgCodeEnum twListEntities(const char * entityName, twInfoTable * params, twInfoTable ** content, char files) {
	...
	char * realPath = NULL;
	char * nameMask = NULL;
    if (getPaths(params, entityName, &path, &realPath, NULL, NULL, NULL, NULL, NULL, NULL, FALSE)) {
			...
			if ((files && !tmp.isDir) || (!files && tmp.isDir)) {
				if ((files && IsWildcardMatch(nameMask, tmp.name, TRUE)) || ((tmp.isDir && strcmp(tmp.name, ".") && strcmp(tmp.name, "..")))) {
					...
					if(tmp.isDir) {
						...
						fullPath = (char *)TW_CALLOC(strlen(path) + 2, 1);
						if (fullPath) {
							strcpy(fullPath, path);
							strcat(fullPath,"/");
						}
...
int listDirsInInfoTable(char * entityName, char * virtualPath, twInfoTable * it) {
	...
	while (tmp && hnd) {
		...
		fullVirtualParent = (char *)TW_CALLOC(strlen(virtualPath) + 1, 1);
		fullVirtualPath   = (char *)TW_CALLOC(1 + strlen(virtualPath) + 1 + strlen(tmp->name) + 1, 1);
		...
		if (listDirsInInfoTable(entityName, fullVirtualPath, it)) {
	...
	*content = twInfoTable_Create(ds);
	...
	/* Perform the function */
	{
		...
		while (le && le->value) {
			...
			row = twInfoTableRow_Create(twPrimitive_CreateFromString(f->name, TRUE));
			...
			virtualPath = (char *)TW_CALLOC(1 + strlen(f->virtualPath) + 1, 1);
			strcat(virtualPath, "/");
			...
			if (listDirsInInfoTable((char *)entityName, virtualPath, *content)) {
...
enum msgCodeEnum twDeleteFile(const char * entityName, twInfoTable * params, twInfoTable ** content) {
	...
	*content = NULL;
	...
	if(!f || isTimedOut){
		...
		TW_LOG(TW_DEBUG, "twDeleteFile: File is currently in Use", realPath);
	}
	cleanUpPaths(&path, &realPath, NULL, NULL, NULL, NULL, NULL, NULL);
	return TWX_SUCCESS;
}
...
enum msgCodeEnum twGetFileInfo(const char * entityName, twInfoTable * params, twInfoTable ** content) {
    ...
    if (getPaths(params, entityName, &path, &realPath, NULL, NULL, NULL, NULL, &adjustedPath, &realAdjustedPath, FALSE)) {
...
enum msgCodeEnum twMoveFile(const char * entityName, twInfoTable * params, twInfoTable ** content) {
	...
	char * adjustedPath = NULL;
	twFile * f = NULL;
	if (getPaths(params, entityName, NULL, NULL, &sourcePath, &realSourcePath, &targetPath, &realTargetPath, NULL, NULL, FALSE)) {
...
enum msgCodeEnum twWriteToBinaryFile(const char * entityName, twInfoTable * params, twInfoTable ** content) {
	...
	*content = NULL;
	/* Execute the service */
	{
		...
		res = TW_FSEEK(f->handle, (uint64_t)offset, SEEK_SET);
		if (res) {
			TW_LOG(TW_ERROR, "twWriteToBinaryFile: Error seeking in file %s to %lli.  Error: %d", adjustedPath, offset, twDirectory_GetLastError());
			cleanUpPaths(&path, &realPath, NULL, NULL, NULL, NULL, &adjustedPath, &realAdjustedPath);
			twFile_SetIsInUse(f, FALSE);
			return TWX_INTERNAL_SERVER_ERROR;
		}
...
enum msgCodeEnum twStartFileTransfer(const char * entityName, twInfoTable * params, twInfoTable ** content) {
	...
	twFile * f = NULL;
	char * mode = NULL;
	if (getPaths(params, entityName, &path, &realPath, NULL, NULL, NULL, NULL, &adjustedPath, &realAdjustedPath, TRUE)) {
...
enum msgCodeEnum twFinishFileTransfer(const char * entityName, twInfoTable * params, twInfoTable ** content) {
	...
	TW_LOG(TW_DEBUG, "twFinishFileTransfer: Finishing file transfer %s", ft->transferId ? ft->transferId : "tid missing");

	/* Outputs */
	*content = NULL;
	/* Perform the function */
	f = twFileManager_GetOpenFile(entityName, NULL, NULL, ft->transferId, NULL);
	if (!f){
		TW_LOG(TW_ERROR, "twFinishFileTransfer: File: %s not found in open files list", ft->targetFile);
		twFileTransferInfo_Delete(ft);
		return TWX_INTERNAL_SERVER_ERROR;
	}
	TW_LOG(TW_AUDIT, "twFinishFileTransfer: %s.  File: %s", ft->state, f->realPath);
	twFileManager_CloseFile(f);
