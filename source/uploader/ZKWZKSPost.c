...
int main(int argc, char *argv[])
{
    ...
    ret = sgsGetDataInfoFromBufferPool("ZKWZKSCollector", &(bufferInfo[0]));
    if(ret == -1)
    {
    ...
    ret = sgsInitDataInfo(NULL, &(dInfo[0]), 0, "./conf/Collect/ZKWZKSCollector", bufferInfo[0].shmId, &numberOfData[0]);
    if(ret == -1)
    {
...
*/
    ...
    while(1) 
    {
        ...
        if(1)//(now-last) >= interval + 4 )
        {
            ...
            now = last;
            last -= 4;
            //sgsSendQueueMsg(eventHandlerId, "[Control];SGSlogger;ZKWZKSCollector;SaveLogNow", EnumLogger);
#else
            ...
            fgets(buf, 1023, fp);
            printf("buf is \n%s\n",buf);
            SetConfig(buf, "203.73.24.133:8443/windfarm");
#endif
...
int GetConfig()
{
    ...
    snprintf(postConfig.IP[i], 127, "203.73.24.133:8443/windfarm");
    ...
    snprintf(postConfig.Station_ID, 15, "WindTest");
    snprintf(postConfig.GW_ID, 15, "01");

    fp = fopen("./conf/Upload/ZKWZKSPost", "r");

    if(fp == NULL)
    {

        printf("ZKWZKSPost can't open the setting file. I'll use the default value\n");
        return 0;
...
int PostToServer()
{
    ...
    while(tempInfo[0] != NULL)
    {
        ...
        if(dLog.valueType == INTEGER_VALUE)
        {
            ...
            snprintf(buf, sizeof(buf) -1, "%i", dLog.value.i);

        }
        else if(dLog.valueType == LONGLONG_VALUE)
        {
        ...
        else
        {
            ...
            snprintf(buf, sizeof(buf) -1, "%s", dLog.value.s);

        }

        cJSON_AddStringToObject(root, tempInfo[0]->valueName, buf);

        tempInfo[0] = tempInfo[0]->next;

    }

    cJSON_AddStringToObject(root, "WindFarmType", "ZKW");
