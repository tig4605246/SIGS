...
int main(int argc, char *argv[])
{
    ...
    ret = sgsInitControl("aemdra_agent");
    if(ret < 0)
    {

        printf("aemdra_agent aborting\n");
        return -1;
    ...
    *tempPt = 0;
...

    {
        ...
        if(strlen(buf) > 0)
        {
            ...
            cJSON_AddStringToObject(field,"GWID",gwId);//Add GWID to object first
...
ssize_t process_http( char *content)
{
    ...
    snprintf(buf, sizeof(buf) - 1, "sendline : \n %s\n",sendline);
    AddToLogFile(logPath, buf);
    ...
    if(error != NULL)
    {
        ...
        snprintf(buf, sizeof(buf) - 1, "\033[1;32m""[%s,%d] Post Result : %s \n""\033[1;37m",__FUNCTION__,__LINE__,recvline);
        AddToLogFile(logPath, buf);
    ...
    else
    {
        ...
        snprintf(buf, sizeof(buf) - 1, "\033[1;31m""[%s,%d] %s\n""\033[1;37m",__FUNCTION__,__LINE__, recvline);
        AddToLogFile(logPath, buf);
        close(sockfd);
        return -1;
...
void stopAndLeave(int sigNum)
{
    ...
    char buf[256];
    snprintf(buf, sizeof(buf) - 1, "aemdra_agent is quitting...\n");
    AddToLogFile(logPath, buf);
    exit(0);
    return;
