/*
    ...
    Program statement: 
        Upload stored data in the shared memory to the Mongodb. This is made for the old version (SEGV21).
...
int CreateJSONAndRunHTTPCommand(int IndexNum, char *dcode)
{
    ...
    if((fd=open(filename, O_RDONLY)) < 0)
    {
        ...
        syslog(LOG_ERR, "[%s:%d] fail in opening %s log file",__FUNCTION__, __LINE__, filename);
        sprintf(buf, "%s/invalid/U-%06d-open.inv", LOG_PATH_127, IndexNum);
        if(rename(filename, buf) != 0)
            syslog(LOG_ERR, "[%s:%d] fail in removeing %s log file",__FUNCTION__, __LINE__, filename);
        return -1;
...
int UploadDataProcess(void)
{
    ...
    do{
        ...
        if(ret != sizeof(iven))
        {
            ...
            syslog(LOG_ERR, "[%s:%d] fail in reading %s log file",__FUNCTION__, __LINE__, filename);
